...
---
# PMSM_Torque_Current Analysis
The `PMSM_Torque_Current` class is a specialized analysis for Permanent Magnet Synchronous Machines (PMSM) that calculates the torque and current characteristics based on the motor's design and operating conditions. It corresponds to the [Torque vs Current analysis](/docs/docs/Analysis_PMSM/TorqueVsCurrent) checkpoint in the eMotorSolution GUI.
The `PMSM_Torque_Current` class inherits from the [`MS_InputControl`](/docs/api/Analyses/InputControl) base class, which provides common functionality for managing analysis input data. It is accessible through `eMotorSolution.CheckPoints.Analysis.PMSM.Torque_Current.PMSM_Torque_Current`.
...
- **phases_expression**: `str` A mathematical expression defining the list of phase values for the analysis.
- **phases_unit**: `Literal["deg", "rad"]` The unit of the phases expression.
...
| set_phases(value, unit) | Sets the phases expression with `value: str \| List[float]` and `unit: Literal["deg", "rad"]`. |
...
| `phases` | `List[float]` The list of phase values in `deg`. :warning: *read-only* |
