...
トランジェント解析（過渡解析）は、時間に依存する電磁現象をシミュレートするための方法です。任意の電流波形や電圧波形を与えた解析が可能です。

:::info
現状の機能では，TP-EEC法の設定には対応できていません。
また，例えば外部トルクを印加した負荷応答解析や脱調模擬解析（Dynamic module使用）は対応していません。
将来的にこれらの機能もサポートする予定です。
:::


## Transientの作成

トランジェント解析（過渡解析）を追加するには、プロジェクトツリー内の `Analysis` マイルストーンを右クリックし、`Transient` を選択します。これにより、`Analysis` マイルストーンの下にユニークな名前を持つトランジェント解析ノードが作成されます。プロジェクトディレクトリ内には指定された名前の新しいフォルダが作成され、EMSolution 用の入力制御ファイル（JSON形式）が生成されます。
